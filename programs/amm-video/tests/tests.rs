...
fn test_initialize() {
    ...
    let instruction = create_initialise_ix(
        &mut svm, &payer, mint_x, mint_y, config, mint_lp, vault_x, vault_y, treasury_x, treasury_y,
    );
...
pub fn test_swap() {
    ...
    let swap_ix = create_swap_ix(
        &mut svm, &payer, mint_x, mint_y, mint_lp, config, vault_x, vault_y, treasury_x, treasury_y,
    );

    let res = send(&mut svm, &[init_ix, deposit_ix, swap_ix], &payer, &[&payer]);
    assert!(res.is_ok(), "{res:?}");

    let treasury_account = svm.get_account(&treasury_x).unwrap();
    let collected_fee = u64::from_le_bytes(treasury_account.data[64..72].try_into().unwrap());
    assert_eq!(collected_fee, 30_000);
}
